...
| Guarda de auto-asociación | Se descartan los pares donde `fromId === toId`, con warning, **sólo cuando el padre es contact-shaped** (`parentObjectType === 'contact'`); con un padre company la coincidencia numérica de ids es casualidad entre dos espacios de ids distintos y el par se asocia igual | Cuando el BP es una persona, un ContactEmployee puede resolver al **mismo** contacto de HubSpot (mismo email). Asociar un contacto a sí mismo es basura o un rechazo de la API, y este caso no existía cuando el padre era siempre una company |
...
**Guarda de auto-asociación**, presente en los dos caminos pero **compuertada por el tipo del padre**: se descarta el par cuando `parentObjectType === 'contact'` **y** `fromId === toId`, con un warning. La compuerta no es opcional: el id de HubSpot de una company y el de un contact viven en espacios de ids de tipos de objeto distintos, así que con un padre company una coincidencia numérica entre ambos es pura casualidad y descartar el par ahí perdería en silencio una asociación legítima que ya funcionaba. La comparación de ids sólo significa "es el mismo objeto" cuando ambos lados son contact-shaped. Caso nuevo que no podía ocurrir con un padre company.
...
4. Devuelve si el flavor del tenant no es B1, resuelto con `resolveSapFlavor({ tenantModels })` de `src/infrastructure/config/SapFlavorConfigRepository.js` — la misma función que usa el guard de flavor del spec hermano. **En producción esta rama es inalcanzable** y queda sólo como defensa en profundidad: la compuerta de flavor terminó viviendo también en `getPropertiesFlagsConfig`, así que en un tenant no-B1 la config ya llega apagada y el enricher sale en el paso 3 sin llegar a mirar el flavor. Los tests del enricher sí la alcanzan porque inyectan la config a mano.
...
| Tenant con flavor S/4 | Todo lo nuevo es no-op + un aviso una vez por corrida. **El aviso NO sale del enricher:** la compuerta de flavor vive en `getPropertiesFlagsConfig` (el repositorio), así que la config de un tenant no-B1 resuelve a "apagado" **antes** de que la comprobación de flavor propia del enricher llegue a correr — el enricher sale antes, en su chequeo de `hubspotProperty`. En producción esa rama del enricher es **inalcanzable**, y el aviso real es un `console.warn('propertiesFlags ignorada: solo aplica a SAP B1', { sapFlavor })` dentro de `BusinessPartnerCreationConfigRepository` |
...
| `fromId === toId` (auto-asociación) **con padre contact** | Se descarta el par + warning; el contacto ya quedó creado/actualizado |
| `fromId === toId` con padre **company** | **No** se descarta: son espacios de ids distintos (company vs contact), la coincidencia es casualidad y el par se asocia normalmente |
