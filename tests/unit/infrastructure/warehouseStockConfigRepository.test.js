...
describe('WarehouseStockConfigRepository', () => {
  ...
  it('defaults to the B1 strategy with no fields/exclusions when nothing is configured', async () => {
    await expect(repository.getWarehouseStockConfig({ tenantModels: models({}) }))
      .resolves.toEqual({ strategyName: 'b1_ItemWarehouse', rawFields: null, rawExclusions: null, rawAvailableFormula: null });
  });
  ...
  it('reads the configured strategy, fields and exclusions', async () => {
    ...
    })).resolves.toEqual({ strategyName: 's4_PlantStorageLocation', rawFields, rawExclusions, rawAvailableFormula: null });
  });
  ...
  it('reads with a plain findOne per key and never upserts (unlike tenantConfiguration.service.getValue)', async () => {
    ...
    expect(tenantModels.Configuration.findOne).toHaveBeenCalledTimes(4);
    ...
    expect(tenantModels.Configuration.findOne).toHaveBeenCalledWith({ key: WAREHOUSE_AVAILABLE_FORMULA_CONFIG_KEY });
  });
